...
<head>
  <meta charset="UTF-8">
  <title>{{ isset($mahasiswa) ? 'Edit' : 'Tambah' }} Mahasiswa - SiaCentral</title>
...
<div class="main">
  ...
  <div class="content">
    <div class="page-header"><h1>{{ isset($mahasiswa) ? 'Edit' : 'Tambah' }} Mahasiswa</h1></div>

    <div class="form-card" style="max-width:680px">
      <div class="form-card-header">
        <div class="icon"><i class="bi {{ isset($mahasiswa) ? 'bi-pencil-square' : 'bi-person-plus' }}"></i></div>
        <h2>{{ isset($mahasiswa) ? 'Edit Data Mahasiswa' : 'Data Mahasiswa Baru' }}</h2>
      </div>

      <form method="POST" action="{{ isset($mahasiswa) ? route('admin.mahasiswa.update', $mahasiswa) : route('admin.mahasiswa.store') }}">
        @csrf
        @if(isset($mahasiswa))
          @method('PUT')
        @endif
        <div class="form-card-body">
          ...
          <div class="form-group">
            ...
            <div class="input-wrap"><i class="bi bi-hash"></i><input type="text" name="nim" class="form-control-custom {{ isset($mahasiswa) ? 'readonly-input' : '' }}" placeholder="Contoh: 241011065" value="{{ old('nim', $mahasiswa->nim ?? '') }}" @readonly(isset($mahasiswa)) required></div>
            <div class="form-hint">{{ isset($mahasiswa) ? 'NIM tidak diubah agar data akademik tetap terhubung.' : 'Nomor Induk Mahasiswa harus unik' }}</div>
          </div>

          <div class="form-group">
            <label>Nama Mahasiswa <span>*</span></label>
            <div class="input-wrap"><i class="bi bi-person"></i><input type="text" name="nama_mahasiswa" class="form-control-custom" placeholder="Nama lengkap mahasiswa" value="{{ old('nama_mahasiswa', $mahasiswa->nama_mahasiswa ?? '') }}" required></div>
        ...
        <div class="form-footer">
          ...
          <button type="submit" class="btn-save"><i class="bi bi-check-lg"></i> {{ isset($mahasiswa) ? 'Simpan' : 'Tambah' }}</button>
        </div>
      </form>
    </div>
  </div>
